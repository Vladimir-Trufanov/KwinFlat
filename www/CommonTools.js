...
// ****************************************************************************
// *   Расчитать фактический интервал события и обновить начальное время      *
// ****************************************************************************
function IntEvent(Name,tTimer,ndigit=3,iConsoleLog=false) 
{
...
function SmarttodoError(jqXHR,exception) 
{
   if (jqXHR.status === 0) 
   {
      messi='[000] Нет соединения.';
   } 
   else if (jqXHR.status == 404) 
   {
      messi='[404] Требуемая страница не найдена.';
   } 
   else if (jqXHR.status == 500) 
   {
      messi='[500] Внутренняя ошибка сервера.';
   } 
   ...
   {
      messi='['+jqXHR.status+'] Неперехваченная ошибка: '+jqXHR.responseText;
   }
   return messi;
}
// ****************************************************************************
// *      Выделить метку (наборы символов 'ghjun5' до и после) в принятом     *
// *                    сообщения и извлечь сообщение.                        *
// *    (сейчас, после 14.01.2025 метка требуется для того, чтобы отделить    *
// * реальный ответ сервера на АЯКС-запрос от ошибки PHP в коде формирования. *
// * Ранее, с 19.01.2023 перед сообщением подвешивался сам js-скрипт запроса. *
// ****************************************************************************
NoFresh='NoFresh';

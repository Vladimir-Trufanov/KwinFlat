...
// v2.0.2, 06.04.2025                                 Автор:      Труфанов В.Е.
...
/*
function FirstNum($imgDir,$incount)
{
   $count = $incount;
   // Инициируем номер текущей цифры
   if ($imgDir==imgDigits) if ($incount > 9) $count=0;
   // Инициируем номер фотографии контроллера
   if ($imgDir==imgESP32CAM) 
   {
      if ($incount > 55) $count=40;
      if ($incount < 40) $count=40;
   }
   // Инициируем номер кадра бегощей девочки
   if ($imgDir==imgMulti) if ($incount > 20) $count=1;
   return $count;
}
*/

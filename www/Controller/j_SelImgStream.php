<?php
// PHP7/HTML5, EDGE/CHROME/YANDEX                    *** j_SelImgStream.php ***

// ****************************************************************************
// *                 Сделать ajax-запрос на выборку последнего,               *
// *                    принятого, изображения видеопотока                    *
// ****************************************************************************

// v1.0.1, 04.04.2025                                 Автор:      Труфанов В.Е.
// Copyright © 2025 tve                               Дата создания: 13.03.2025

// Инициализируем корневой каталог сайта, прежние значения времени кадра и его номера в секунде
define ("SiteHost", $_POST['sh']);
$time=$_POST['time'];
$frame=$_POST['frame'];
// Подключаем объект для работы с базой данных моего хозяйства
require_once "../Common.php";  
require_once "../TKvizzyMaker/KvizzyMakerClass.php";
$Kvizzy=new ttools\KvizzyMaker(SiteHost);
// Подключаемся к базе данных
$pdo=$Kvizzy->BaseConnect();
// Выбираем изображение из базы данных
$table=$Kvizzy->SelImgStream($pdo,$time,$frame);
//$mess="data:image/jpeg;base64,";
// Возвращаем сообщение
$message='{"time":'.$table["time"].',"frame":'.$table["frame"].',"fate":'.$table["fate"].',"src":"'.$table["src"].'"}';
echo $message;
exit;

// <!-- --> ******************************************** j_SelImgStream.php ***
